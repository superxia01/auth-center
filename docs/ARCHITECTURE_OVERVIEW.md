...
├─ 用途: 应用服务器
├─ 部署内容:
│   ├─ 前端 (Vite/Next.js) - Nginx/PM2
│   └─ 后端 (Go API) - Systemd
└─ 业务系统:
    ├─ os.crazyaigc.com (账号中心)
    ├─ pr.crazyaigc.com (PR业务)
    ├─ pixel.crazyaigc.com (AI生图)
    ├─ study.crazyaigc.com (知识库)
    └─ crm.crazyaigc.com (客户管理)
...
└─ 数据库:
    ...
    ├─ study_business_db (知识库)
    └─ crm_business_db (客户管理)

🔗 连接方式:
├─ 前端 → 后端: localhost (同一服务器)
└─ 后端 → 数据库: 跨服务器 (上海 → 杭州)
```
